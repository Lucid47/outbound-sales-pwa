...
대상 단계에서는 연락처 개인 다중 선택과 연락처 그룹 일괄 선택을 추가할 수 있습니다. 이 연락처는 고객리스트로 자동 가져오지 않고 `GroupMessageTarget` 임시 스냅샷으로 합쳐집니다. 발송 전 확인 단계의 예약은 `UNCalendarNotificationTrigger`를 사용하며, 알림에서 사용자가 발송 시작을 확인한 뒤 기존 Shortcuts payload를 생성합니다. 예약 알림은 생성 기기에만 귀속하고 Google Drive에서 복원한 다른 기기에는 자동 등록하지 않습니다.

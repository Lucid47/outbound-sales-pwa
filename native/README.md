...
- 방문 히스토리
  ...
  - 텍스트 메모, 사진 메모, 음성 메모
  - 고객 상세에서 최근 텍스트 메모/음성 메모 목록과 상세 보기
  - 음성 메모 상세에서 녹음 재생과 전사 텍스트 확인
  - 음성 메모 저장 후 Apple Speech 기반 전사 상태 갱신
- 설정 탭의 문자 템플릿 추가/수정/삭제
- 연락처 관리
  - 고객리스트를 iOS 연락처 그룹으로 일괄 등록
  - 등록 작업별 앱 소유 연락처와 기존 연결 연락처를 분리해 저장
  - 그룹만 삭제 또는 앱이 신규 생성한 연락처와 그룹을 함께 삭제
  - 기존 연락처, 다른 그룹 소속, 등록 후 변경된 연락처 자동 보호
  - 삭제 소유권과 처리 결과를 로컬 및 Google Drive 백업에 포함
- JSON 백업 내보내기/가져오기
- Google Drive 동기화/백업
...
`GroupSmsTestView`는 단축어와 발송 payload를 검증하는 진단 화면이며 기본 사용자 흐름에 직접 노출하지 않습니다. 실제 고객용 `GroupSmsCampaignView`는 이동 중 조작을 위해 화면상으로는 `대상 → 메시지 → 확인` 세 단계만 표시합니다. 자동화 준비는 상단 상태 배너, 발송 진행과 결과·복구는 실행 후 상태 화면으로 구성해 제품의 6단계 상태는 유지합니다.

단체문자는 하단의 여섯 번째 탭으로 만들지 않습니다. 고객 탭의 `단체문자` 빠른 실행은 현재 고객리스트를, 오늘 탭의 `오늘 고객에게 단체문자`는 오늘 스케줄을 기본 대상으로 전달합니다. 설정과 진행 상태는 작업을 막는 오류만 전면에 표시하고 상세 진단은 화면 우측 공구 아이콘에서 엽니다.

대상 단계에서는 연락처 개인 다중 선택과 연락처 그룹 일괄 선택을 추가할 수 있습니다. `전체 선택`과 `전체 해제`로 현재 범위를 일괄 변경한 뒤 필요한 고객만 개별 선택할 수 있습니다. 이 연락처는 고객리스트로 자동 가져오지 않고 `GroupMessageTarget` 임시 스냅샷으로 합쳐집니다. 발송 전 확인 단계의 예약은 `UNCalendarNotificationTrigger`를 사용하며, 알림에서 사용자가 발송 시작을 확인한 뒤 기존 Shortcuts payload를 생성합니다. 예약 알림은 생성 기기에만 귀속하고 Google Drive에서 복원한 다른 기기에는 자동 등록하지 않습니다.
...
최초 연결에서 받은 Google refresh token은 iOS Keychain에 `ThisDeviceOnly` 정책으로 저장합니다. 이후 Drive 동기화, 저장, 복원, 일반 백업 생성은 로그인 화면을 다시 열지 않고 Keychain의 refresh token으로 access token을 자동 갱신합니다. 계정 이메일과 이름은 화면 표시용으로만 UserDefaults에 저장하며 인증 수단으로 사용하지 않습니다. 2026-07-13 이전 빌드에서 업데이트한 사용자는 기존 refresh token이 저장되어 있지 않으므로 한 번 다시 연결해야 합니다.

`native/GoogleDriveOAuth.xcconfig`가 로컬 파일을 선택적으로 포함하므로 같은 Mac의 Xcode 직접 빌드에서도 OAuth 값이 자동 적용됩니다. 로컬 파일이 없는 새 환경에서는 Drive 로그인이 비활성화된 상태로 빌드됩니다.
...
- 실기기에서 최초 Google 재연결 후 연속 동기화와 앱 재실행 뒤 무로그인 토큰 갱신 확인

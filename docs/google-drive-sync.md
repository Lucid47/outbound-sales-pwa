...
- 관리 기간, 명시적 활동 이벤트, 고객 대시보드 상태 변경 로그를 ID 기준으로 병합합니다.
- 고객리스트 영구삭제 tombstone은 삭제 시각을 함께 저장하며, 병합 시 같은 ID의 오래된 리스트보다 삭제 상태를 우선합니다.
- 개별 고객 영구삭제 tombstone도 같은 방식으로 적용하며 전체 복원과 선택 복원 직후 삭제 고객 및 연결 데이터를 다시 제거합니다.
- 삭제 고객이 포함된 과거 단체문자 캠페인은 감사·발송 기록을 위해 보존하되 수신자 스냅샷의 고객 ID 연결만 해제합니다.
- 같은 ID가 양쪽에 있으면 더 최근 수정 시각을 우선합니다.
- 로그성 데이터는 ID 기준으로 합쳐 데이터 유실을 줄입니다.

## 현재 제한

- 고객리스트와 개별 고객 영구삭제에는 tombstone 정책을 적용했습니다. 방문·메모 같은 개별 기록 단위 삭제에는 아직 공통 tombstone이 없습니다.

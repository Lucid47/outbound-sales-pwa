...
정확한 원본은 `src/App.css`와 `src/App.tsx`입니다. 이 문서는 네이티브 앱 개발자가 CSS를 보지 않고도 주요 화면을 재현할 수 있도록 설명한 보조 사양입니다.
...
- 고객 카드보다 압축된 리스트 형태입니다.
- 첫 줄은 고객명과 상태 pill을 나란히 둡니다.
- 주소와 보조 텍스트는 말줄임 처리합니다.
- 액션 버튼 높이는 카드형보다 낮게 둡니다.

PC 목록형:

- 넓은 행 형태입니다.
- 고객 정보, 상태, 액션 버튼들이 한 행에 더 촘촘하게 배치됩니다.

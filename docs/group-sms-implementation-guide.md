...
이 문서는 `소희가 간다`의 단체문자 기능을 다시 구현하거나 Android로 포팅할 때 바로 사용할 수 있는 기술 기준이다.
...
`localReference`에는 절대 경로나 고객이 볼 수 있는 개인정보를 동기화하지 않는다. 백업 시 파일을 별도 자산으로 저장하고 복원 후 새 로컬 참조를 만든다.
...
1. 앱에서 `PhotosPicker` 또는 파일 선택기로 파일 선택
2. 앱 전용 저장소에 복사
3. SHA-256, MIME type, 크기 기록
4. `AppIntent`로 `IntentFile` 반환
5. Shortcuts에서 `소희가 간다 첨부파일 가져오기` 액션 실행
6. 반복 안의 `메시지 보내기`에 반환된 파일 사용
7. 캠페인 완료/취소 후 임시파일 정리

1차 제품 범위는 캠페인 공통 사진 1장으로 제한한다. 여러 파일과 수신자별 파일은 별도 단계로 확장한다.
...
4. 사진 1장 단축어 단위 검증
5. `AppIntent`/`IntentFile` 첨부파일 브리지 구현

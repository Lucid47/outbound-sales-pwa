...
이 두 동작은 반드시 `반복 종료` 아래, 즉 반복문 바깥에 둔다. 반복문 안에 넣으면 수신자마다 앱이 열려 발송 흐름이 중단된다.

앱에서 단체문자를 시작할 때는 `shortcuts://x-callback-url/run-shortcut`의 `x-success`도 함께 전달한다. 정상 흐름은 다음과 같다.

1. 소희야 가자에서 발송을 확인한다.
2. iOS가 Shortcuts를 전면에 열어 단축어를 실행한다.
3. 반복 발송이 끝나면 완료 callback이 소희야 가자를 다시 연다.
4. 앱의 진행 화면이 `발송 요청 완료`로 바뀐다.

iOS 공개 API로 다른 앱인 Shortcuts를 화면 전환 없이 조용히 실행할 수는 없다. 따라서 “소희야 가자 화면을 계속 유지하면서 백그라운드에서 Shortcuts 실행”이 아니라, “Shortcuts로 잠시 전환한 뒤 자동으로 소희야 가자로 복귀”가 지원 동작이다.

자동 복귀하지 않고 Shortcuts 보관함에 머무르면 다음을 확인한다.

- 설치된 앱의 URL scheme에 `com.lucid47.outboundsales`가 등록되어 있는지
- 마지막 `텍스트`와 `URL 열기`가 반복문 바깥에 있는지
- 완료 URL의 `CampaignId`가 파란색 매직 변수인지, 단순 글자인지
- 앱 진행 화면의 `결과를 받지 못했나요?`에서 상태를 안전하게 복구할 수 있는지

...
1. 현재 `소희야 가자`에 고객관리 데이터와 연결된 단체문자 기능을 완성한다.
...
현재 단계에서 별도 저장소나 별도 앱 타깃을 바로 만들지는 않는다. 먼저 Group SMS를 제품 중립적인 Swift Package와 `소희야 가자` 전용 어댑터로 분리하고, 실기기 검증이 끝난 뒤 독립 앱 저장소 또는 앱 타깃으로 옮긴다.
...
| 단계 | 상태 | 비고 |
| --- | --- | --- |
| 0. 기준선 고정 | 완료 | 본인 번호 반복 발송과 Core 회귀 테스트 보존 |
| 1. 공통 코어 분리 | 진행 중 | 제품 중립 대상·설정·제외 분류·사전점검 완료. 캠페인 저장소 분리 남음 |
| 2. 자동화 준비 | 진행 중 | 준비 상태 모델과 진단 화면 저장 연결 완료. 전용 제품 화면 남음 |
| 3. 대상·작성 | 대기 | 현재 긴 진단 Form을 제품 흐름으로 교체 예정 |
| 4. 사전점검 | 진행 중 | 대상·정책·첨부·예상시간 Core 완료. 제품 화면 남음 |
| 5. 발송 진행 | 대기 | 대상별 진행 bridge 실기기 검증 필요 |
| 6. 결과·복구 | 대기 | 수신자 상태와 안전 재시도 구현 필요 |
| 7. 다중 첨부 | 진행 중 | 첨부 배열 payload 모델 완료. 사진·파일 선택 UI와 App Intent bridge 남음 |
| 8. 제품화 | 대기 | 앞 단계 Gate 통과 후 시작 |
...
`소희야 가자`와 독립 앱은 서로 다른 설정 인스턴스를 주입한다. 독립 앱은 별도 Bundle ID, URL scheme, 단축어 이름을 사용한다.
...
- `소희야 가자`: `Customer`를 `GroupMessageTarget`으로 변환
...
`소희야 가자`는 기존 JSON/Drive 저장 어댑터, 독립 앱은 별도 SwiftData 또는 파일 저장 어댑터를 구현할 수 있다.
...
- 기존 문자 템플릿
- 본문과 개인화 필드
- 사진 여러 장과 일반 파일 여러 개를 함께 선택할 수 있는 첨부 목록
- 대상별 미리보기
- 특정 대상의 최종 문구만 수정
- 고정/랜덤 간격과 묶음 휴식
- 초안 저장
...
목표: 한 캠페인에 사진 여러 장과 일반 파일 여러 개를 첨부하고, 같은 첨부 목록을 각 수신자에게 안정적으로 전달한다.

구현 원칙:

- 데이터 모델과 UI는 처음부터 첨부 배열을 사용한다.
- 사진과 파일을 섞어 선택할 수 있다.
- 사진앱은 다중 선택, 파일앱은 다중 선택을 지원한다.
- 선택 순서를 유지하고 사용자가 순서를 바꾸거나 개별 삭제할 수 있다.
- 지원 개수, 개별 크기와 전체 크기 제한은 전송 방식별 실기기 결과로 결정한다.
- 제한을 초과하면 자동 압축 또는 발송 전 차단 중 하나를 명확히 표시한다.

작업:

- `GroupSmsAttachment` 배열 모델 추가
  - ID, 사진/파일 종류, 원본 파일명, UTType/MIME type, 크기, 순서, checksum, 로컬 참조
- `PhotosPicker`의 다중 사진 선택과 파일 선택기의 다중 파일 선택 연결
- 선택한 모든 첨부를 앱 전용 캠페인 저장소에 복사
- 파일 유형, 개별 크기, 전체 크기, 존재 여부 사전검사
- 썸네일 또는 파일 아이콘, 파일명, 용량, 삭제와 순서 변경 UI 제공
- Shortcuts에서 읽을 수 있는 App Intent 또는 임시 파일 전달로 첨부 배열 반환
- 첨부 권한 전용 시험
- 성공·취소·만료 시 임시 파일 정리
- 백업 포함 여부와 보관기간 설정
...
- 사진 1장, 사진 여러 장, 파일 1개, 파일 여러 개, 사진+파일 혼합 발송 시험
- 본문+다중 첨부와 첨부만 발송 시험
- 수신자마다 같은 개수와 순서로 첨부되는지 확인
- 지원하지 않는 파일은 성공으로 기록하지 않고 사전점검 또는 오류로 분류
- 앱 재실행과 중단 후 임시 파일 누수 없음

### 단계 8: 제품화 준비

`소희야 가자` 통합 기능이 아래 조건을 만족하면 독립 앱 작업을 시작한다.

- 텍스트 캠페인 20회 이상 회귀 시험
- 중단/미확정/재시도 시험 통과
- 다중 사진, 일반 파일과 혼합 첨부 시험 통과
- 새 기기 설치 온보딩 시험 통과
...
| F. 첨부 | 다중 사진·파일·혼합 목록, 권한, 임시파일 | 모든 첨부 실제 수신 |
